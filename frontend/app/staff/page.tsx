...
export default function StaffDashboard() {
  ...
  // When a report is selected, find its duplicates
  const handleSelect = async (r: Report) => {
    setSelected(r)
    setDuplicateReports([])
    if (r.duplicate_count > 0) {
      // Find all reports that link to this one as original
      try {
        const all = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/reports/department/${encodeURIComponent(r.department)}`)
        const dupes = all.data.filter((rep: Report) => rep.original_report_id === r.id)
    ...
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .detail-panel { position:fixed; right:0; top:0; bottom:0; width:420px; background:white; border-left:1px solid var(--border); z-index:200; transform:translateX(100%); transition:transform 0.25s ease; display:flex; flex-direction:column; box-shadow:-8px 0 24px rgba(0,0,0,0.06); }
        ...
        .detail-body { padding:0; flex:1; overflow-y:auto; display:flex; flex-direction:column; }
        ...
        .detail-footer { padding:20px 24px; border-top:1px solid var(--border); flex-shrink:0; }
        ...
        .maps-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 16px; background:#4285f4; color:white; border-radius:6px; font-size:12px; font-weight:500; text-decoration:none; transition:background 0.15s; }
        .maps-btn:hover { background:#3367d6; }
        .urgent-banner { background:#fef2f2; border:1px solid #fecaca; border-radius:6px; padding:12px 16px; display:flex; align-items:flex-start; gap:10px; }
        .duplicate-banner { background:#fff7ed; border:1px solid #fde68a; border-radius:6px; padding:12px 16px; }
      `}</style>
      ...
      <div style={{minHeight:'100vh',background:'var(--bg)'}}>
        ...
        <div className="content">
          {overdue.length > 0 && (
            <div className="urgent-banner" style={{marginBottom:'16px'}}>
              ...
              <div>
                ...
                <div style={{fontSize:'12px',color:'#991b1b',fontWeight:300}}>
                  These reports have passed their SLA deadline. Please action them immediately.
                </div>
              </div>
            </div>
          )}
              ...
              <table>
                ...
                <tbody>
                  {filtered.map(r => {
                    ...
                    return (
                      <tr key={r.id} className={over?'overdue-row':''} onClick={() => handleSelect(r)}>
                        <td>
                          <div style={{display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap'}}>
                            ...
                            {r.original_report_id && (
                              <span style={{background:'#f3f4f6',color:'#6b7280',fontSize:'10px',fontWeight:500,padding:'1px 6px',borderRadius:'99px'}}>
                                duplicate
                              </span>
                            )}
                          </div>
          ...
          <>
            <div className="detail-header">
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:'18px',fontWeight:600,textTransform:'capitalize',marginBottom:'6px'}}>{selected.issue_type}</div>
                <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                  ...
                  {selected.duplicate_count > 0 && (
                    <span style={{background:'#fff7ed',color:'#d97706',fontSize:'11px',fontWeight:600,padding:'2px 8px',borderRadius:'99px'}}>
                      🔁 {selected.duplicate_count + 1} reports
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="detail-body">
              ...
              {/* Urgency banner for overdue */}
              {isOverdue(selected) && selected.status !== 'resolved' && (
                <div style={{padding:'16px 24px',borderBottom:'1px solid var(--border)'}}>
                  <div className="urgent-banner">
                    <div style={{fontSize:'20px'}}>🚨</div>
                    <div>
                      <div style={{fontSize:'13px',fontWeight:600,color:'#dc2626',marginBottom:'2px'}}>SLA Deadline Passed</div>
                      <div style={{fontSize:'12px',color:'#991b1b',fontWeight:300,lineHeight:1.5}}>
                        This report was due by {new Date(selected.sla_deadline).toLocaleDateString('en-GB',{day:'numeric',month:'long'})}. Please resolve it immediately.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {selected.duplicate_count > 0 && (
                <div style={{padding:'16px 24px',borderBottom:'1px solid var(--border)'}}>
                  <div className="duplicate-banner">
                    ...
                    <div style={{fontSize:'12px',color:'#92400e',fontWeight:300,lineHeight:1.5,marginBottom:'10px'}}>
                      Multiple people have reported the same problem in this area. This is a high-priority issue that needs immediate attention.
                    </div>
                    {duplicateReports.length > 0 && (
                      <>
                        <div style={{fontSize:'11px',letterSpacing:'0.06em',textTransform:'uppercase',color:'#92400e',fontWeight:500,marginBottom:'8px'}}>
                          Additional photos from other reporters:
                        </div>
                        <div className="photo-grid">
                          {duplicateReports.filter(d => d.photo_url).map(d => (
                            <img
                              key={d.id}
                              src={`${process.env.NEXT_PUBLIC_API_URL}${d.photo_url}`}
                              alt="Duplicate report"
                              style={{width:'100%',height:'80px',objectFit:'cover',borderRadius:'4px',border:'1px solid #fde68a'}}
                            />
                          ))}
                        </div>
                      </>
              ...
              <div className="detail-section">
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
                  <div>
                    ...
                    <div className="field-value" style={{color:isOverdue(selected)&&selected.status!=='resolved'?'#dc2626':'#16a34a',fontSize:'13px'}}>
                  ...
                  <div>
                    ...
                    <div className="field-value mono" style={{color: daysUntil(selected.sla_deadline) <= 0 ? '#dc2626' : daysUntil(selected.sla_deadline) <= 1 ? '#ea580c' : '#16a34a'}}>
                      {daysUntil(selected.sla_deadline) <= 0 ? 'OVERDUE' : `${daysUntil(selected.sla_deadline)}d`}
                    </div>
                  </div>
                  <div>
                    <div className="field-label">Filed On</div>
                    <div className="mono" style={{fontSize:'12px',color:'var(--muted)'}}>{new Date(selected.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</div>
                  </div>
                  <div>
                    <div className="field-label">Time</div>
                    <div className="mono" style={{fontSize:'12px',color:'var(--muted)'}}>{new Date(selected.created_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</div>
                  </div>
                </div>
              </div>
              ...
              {selected.description && (
                <div className="detail-section">
                  <div className="field-label">Citizen Description</div>
                  <div style={{fontSize:'14px',color:'#374151',lineHeight:1.7,fontWeight:300,background:'#f9fafb',padding:'12px',borderRadius:'6px',border:'1px solid var(--border)'}}>
                    "{selected.description}"
                  </div>
                </div>
              )}
              ...
              <div className="detail-section">
                ...
                <div style={{fontSize:'13px',color:'#374151',lineHeight:1.5,marginBottom:'10px'}}>{selected.address}</div>
                {selected.latitude && selected.longitude && (
                  <div style={{display:'flex',gap:'8px',alignItems:'center',flexWrap:'wrap'}}>
                    <a href={mapsUrl(selected.latitude, selected.longitude)} target="_blank" rel="noreferrer" className="maps-btn">
              ...
              {selected.original_report_id && (
                <div className="detail-section">
                  ...
                  <div style={{fontSize:'13px',color:'#6b7280',background:'#f3f4f6',padding:'10px 12px',borderRadius:'6px'}}>
                    This is a linked duplicate — it was filed as part of a larger report cluster for the same issue.
                  </div>
                </div>
              )}
            ...
            <div className="detail-footer">
              {selected.status === 'resolved' ? (
                <div style={{textAlign:'center',fontSize:'13px',color:'#16a34a',fontWeight:500,padding:'8px'}}>
