    ...
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        :root {
          --ink:#111827; --accent:#e63329; --muted:#6b7280;
          --border:#f3f4f6; --green:#16a34a; --bg:#f9fafb;
          --white:#ffffff; --sidebar:#111827;
        }
        ...
        .sidebar-item {
          display:flex; align-items:center; gap:10px;
          padding:10px 12px; border-radius:6px; margin:2px 8px;
        ...
        .user-row {
          display:flex; align-items:center; gap:10px;
          padding:10px; border-radius:6px; background:rgba(255,255,255,0.04);
        }
        .user-avatar {
          width:32px; height:32px; background:var(--accent);
        ...
        .stat-value { font-size:32px; font-weight:300; line-height:1; font-family:'DM Mono',monospace; }
        ...
        thead th {
          ...
          color:var(--muted); font-weight:500; padding:12px 16px;
          border-bottom:1px solid var(--border); text-align:left; background:#fafafa;
        }
      ...
      <div className="layout">
        ...
        <aside className="sidebar">
          ...
          {overdue.length > 0 && (
            <div className="sidebar-item" onClick={() => setFilter('overdue')} style={{color:'#fca5a5'}}>
          ...
          <Link href="/map" target="_blank" className="sidebar-item">
            <span className="sidebar-item-icon">🗺️</span>
            Live Map
          </Link>
          <Link href="/scorecard" target="_blank" className="sidebar-item">
            <span className="sidebar-item-icon">📊</span>
            Scorecard
          </Link>
          <Link href="/" target="_blank" className="sidebar-item">
            <span className="sidebar-item-icon">🏠</span>
            Public Site
          </Link>
        ...
        <div className="main">
          ...
          <div className="content">
            ...
            <div className="stats-grid">
              {[
                { label:'Total Reports', value: reports.length, icon:'📁', iconBg:'#f3f4f6', color:'var(--ink)', trend:'All time' },
                { label:'Open Issues', value: open.length, icon:'🔴', iconBg:'#fef2f2', color:'#dc2626', trend:'Awaiting resolution' },
            ...
            <div className="table-wrap">
              <div className="table-header">
                <span className="table-title">
                  {filter === 'all' ? 'All Reports' : filter === 'open' ? 'Open Reports' : filter === 'overdue' ? '⚠ Overdue Reports' : 'Resolved Reports'}
                </span>
                <span className="table-count">{filtered.length} reports</span>
              </div>

              {loading ? (
                <div style={{padding:'48px',textAlign:'center',color:'var(--muted)',fontSize:'14px'}}>Loading...</div>
              ) : filtered.length === 0 ? (
                        ...
                        <tr key={r.id} className={over ? 'overdue-row' : ''} onClick={() => setSelected(r)}>
                          ...
                          <td>
                            {r.status === 'resolved' ? (
                              ...
                              <span className="badge" style={{background:'#fef2f2',color:'#dc2626'}}>⚠ Overdue</span>
                            ) : (
                              <span style={{fontSize:'12px',color: days <= 1 ? '#ea580c' : '#16a34a', fontFamily:'DM Mono,monospace'}}>
                                {days}d left
                              </span>
                            )}
                          </td>
                          <td>
                            <span className="badge" style={{
                              background: r.status==='resolved'?'#f0fdf4':'#fef2f2',
                              color: r.status==='resolved'?'#16a34a':'#dc2626'
                            }}>
                          ...
                          <td onClick={e => e.stopPropagation()}>
                            {r.status==='open' ? (
                              <button className="resolve-btn" onClick={() => resolve(r.id)} disabled={resolving===r.id}>
              ...
              <div className="detail-body">
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
                  <div>
                    ...
                    <span className="badge" style={{background:severityBg(selected.severity),color:severityColor(selected.severity),fontSize:'12px'}}>
                      {selected.severity}
                    </span>
                  </div>
                  <div>
                    <div className="detail-field-label">Status</div>
                    <span className="badge" style={{background:selected.status==='resolved'?'#f0fdf4':'#fef2f2',color:selected.status==='resolved'?'#16a34a':'#dc2626',fontSize:'12px'}}>
              ...
              <div className="detail-footer">
                {selected.status==='open' ? (
                  <button
                    ...
                    style={{width:'100%',padding:'12px',background:'#16a34a',color:'white',border:'none',borderRadius:'8px',fontSize:'14px',fontWeight:500,cursor:'pointer',fontFamily:'DM Sans,sans-serif',transition:'background 0.15s'}}
                  >
                    {resolving===selected.id ? 'Resolving...' : '✓ Mark as Resolved'}
                  </button>
                ) : (
